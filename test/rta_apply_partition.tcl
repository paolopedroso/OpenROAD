...
proc rta_apply_partition { part_file num_parts {overlap_factor 1.0} } {
  ...
  # overlap_factor 1.0 (default) = strict, non-overlapping strips.
  # > 1.0 widens each strip by that factor around its midpoint (clamped to
  # the die). E.g. with overlap_factor 1.5 and num_parts 2, each strip is
  # 75% of die width and the two overlap in the middle 50% -- a softer
  # "guidance" region instead of a hard fence.
  set base_strip_w [expr {double($width) / $num_parts}]
  set strip_w [expr {$base_strip_w * $overlap_factor}]
  set half_overhang [expr {0.5 * ($strip_w - $base_strip_w)}]
  puts "rta_apply_partition: die [list $lx $ly $ux $uy] DBU,\
        $num_parts strips base_w=$base_strip_w eff_w=$strip_w\
  ...
  for {set i 0} {$i < $num_parts} {incr i} {
    ...
    if { $region == "NULL" } {
      ...
      # Strip center is at base_strip_w * (i + 0.5) offsets; widen by
      # half_overhang on each side, then clamp to die.
      set strip_center_x [expr {$lx + ($i + 0.5) * $base_strip_w}]
      set rlx [expr {round($strip_center_x - 0.5 * $strip_w)}]
      set rux [expr {round($strip_center_x + 0.5 * $strip_w)}]
      if { $rlx < $lx } { set rlx $lx }
      if { $rux > $ux } { set rux $ux }
